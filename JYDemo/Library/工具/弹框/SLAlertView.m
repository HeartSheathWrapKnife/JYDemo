...
+ (void)alertViewWithTitle:(NSString *)title cancelBtn:(NSString *)cancelbtn destructiveButton:(NSString *)des otherButtons:(NSArray *)array clickAtIndex:(void (^)(NSInteger))click
{
    ...
    if (title) {
        // 标题
        titleLabel = [UILabel labelWithText:title font:12 textColor:[UIColor darkGrayColor] frame:Rect(0, 0, content.width, 50)];
    ...
    if (des) {
        destructiveButton = [UIButton buttonWithTitle:des titleColor:[UIColor redColor] backgroundColor:[UIColor clearColor] font:15 image:nil frame:Rect(0, desY, content.width, 40)];
