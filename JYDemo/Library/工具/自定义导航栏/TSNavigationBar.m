...
/// 返回按钮图片
#define BackButtonImageName @"icon_nav_bacblack"
#warning 改图片名字在这里改
...
{
    if (self = [super init]) {
        UILabel * label = [UILabel labelWithText:title font:17 textColor:[UIColor blackColor] frame:Rect(40, 20, kScreenWidth - 80, 44)];
        [self addSubview:label];
        label.textAlignment = NSTextAlignmentCenter;
        
        UIButton * back = [UIButton buttonWithTitle:nil titleColor:nil backgroundColor:nil font:0 image:BackButtonImageName frame:Rect(5, 32, 44, 44)];
...
{
    if (self = [super init]) {
        UILabel * label = [UILabel labelWithText:title font:17 textColor:[UIColor blackColor] frame:Rect(0, 20, kScreenWidth, 44)];
        [self addSubview:label];
        self.titleLabel = label;
        label.textAlignment = NSTextAlignmentCenter;
    }
